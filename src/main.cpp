...
public:
	FindGears (Image &m): src_img(m), bin(m.n_rows, m.n_cols),
	...
	void SaveBin(const char *name, bool dt = false)
	{
		uint r,g,b,x;
		Image dst_image(bin.n_rows,bin.n_cols);
		if (!dt)
			for (uint i=0;i<bin.n_rows;i++)
				for (uint j=0;j<bin.n_cols;j++)
				{
					...
					g = (((x%170)-85)*3)%256;
					b = ((x-170)*3)%256;
					dst_image(i,j) = make_tuple(r,g,b);
				}
		else
			for (uint i=0;i<bin_dt.n_rows;i++)
				for (uint j=0;j<bin_dt.n_cols;j++)
				{
					x = bin_dt (i,j);
					r = (((x%170)%85)*3)%256;
					g = (((x%170)-85)*3)%256;
...
private:
	...
	bool CheckGear(uint n, uint ncogs)
	{
		...
		{
			y = midr*sin(2*PI*phi/360);
			x = midr*cos(2*PI*phi/360);
	...
	uint CountCogs2(uint n, bool *whole)
	{
		...
		*whole = true;
		...
		{
			y = middle_r*sin(2*PI*phi/360);
			x = middle_r*cos(2*PI*phi/360);
			if ((bin (cen_y+y,cen_x+x)==n+1)&&(!cog))
			{
				if (!counter)
				{
...
repair_mechanism(Image& in, char **argv)
{
	...
	{
		arg=change_name(argv[1],ntest);
		Image test = load_image(arg);
		FindGears test_img(test);
	...
	arg=change_name(argv[1],result_idx);
	Image test = load_image(arg);
	if (DEBUG)
		cout<<"name "<<arg<<endl;
	uint r,g,b,r1,g1,b1;
	...
	if (DEBUG)
		cout<<"place"<<y<<" "<<x<<endl;
	for (uint i = y - radius; (i<= y + radius)&&(i<in.n_rows); i++)
		for (uint j = x - radius; (j<= x+radius)&&(i<in.n_cols); j++)
		{
			tie(r,g,b) = in (i,j);
			tie (r1,g1,b1) = test (i-(y - radius),j-(x - radius));
			if (r>100)
				in (i,j) = make_tuple(0,0,0);
			if (g1>100)
				in (i,j) = make_tuple(0,g1,0);
		}
	if (DEBUG)
		save_image(in,"debres.bmp");
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        Image dst_image;
        int result_idx;
        tie(result_idx, object_array, dst_image) = repair_mechanism(src_image, argv);
        save_image(dst_image, argv[2]);

        fout << result_idx << endl;
        fout << object_array.size() << endl;
       // const auto &obj = object_array;
